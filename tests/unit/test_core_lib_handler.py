...
class TestHandleSplunkProvidedLib:
    ...
    def test_handle_splunk_provided_lib_calls_both_functions(
        ...
        """Test that handle_splunk_provided_lib calls both _cache_lib and
        _remove_lib_folder."""
...
class TestCacheLib:
    ...
    @patch("solnlib.core_lib_handler.importlib.import_module")
    @patch("solnlib.core_lib_handler._is_module_from_splunk_core")
    def test_cache_lib_not_from_splunk_core(
        self, mock_is_splunk_core, mock_import_module
    ):
        """Test that _cache_lib raises AssertionError when module is not from
        Splunk core."""
        ...
        assert f"The module {lib_name} is not from Splunk core site-packages." in str(
            exc_info.value
        )
        mock_import_module.assert_called_once_with(lib_name)
        mock_is_splunk_core.assert_called_once_with(mock_module)
    ...
    def test_cache_lib_import_error(self, mock_import_module):
        """Test that _cache_lib propagates ImportError when module cannot be
        imported."""
...
class TestRemoveLibFolder:
    ...
    def test_remove_lib_folder_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "solnlib.core_lib_handler._get_app_path", return_value=app_dir
    ...
    def test_remove_lib_folder_no_lib_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Should print an error message about lib directory not existing
            mock_print.assert_called_with(
                "ERROR: Error in _remove_lib_folder for urllib3: [Errno 2] No such file or directory: '"
    ...
    def test_remove_lib_folder_app_path_none(self):
        ...
        with patch("solnlib.core_lib_handler._get_app_path", return_value=None), patch(
            "builtins.print"
        ) as mock_print:
            # Should not raise an exception
            core_lib_handler._remove_lib_folder("urllib3")

        # Should print a warning message about unable to determine app directory path
        mock_print.assert_called_once_with(
            "WARNING: Unable to determine app directory path for urllib3"
        )
    ...
    def test_remove_lib_folder_permission_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_print.assert_called_once_with(
                f"ERROR: Failed to remove library folder {folder_to_remove}: Access denied"
            )
    ...
    def test_remove_lib_folder_empty_lib_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Should not print anything since there are no folders to remove
            mock_print.assert_not_called()

    def test_remove_lib_folder_with_files_not_directories(self):
        """Test _remove_lib_folder ignores files and only processes
        directories."""
        with tempfile.TemporaryDirectory() as temp_dir:
            app_dir = os.path.join(temp_dir, "test_app")
            lib_dir = os.path.join(app_dir, "lib")
...
class TestIsModuleFromSplunkCore:
    ...
    def test_is_module_from_splunk_core_true(self, mock_is_core_path, mock_get_regex):
        ...
        mock_module.__file__ = (
            "/opt/splunk/lib/python3.9/site-packages/splunklib/__init__.py"
        )
    ...
    def test_is_module_from_splunk_core_false_no_match_in_paths(
        self, mock_is_core_path, mock_get_regex
    ):
...
class TestGetCoreSitePackagesRegex:
    ...
    def test_get_core_site_packages_regex_windows(self):
        ...
        assert (
            regex.search(r"C:\Program Files\Splunk\Python-3.9\lib\site-packages")
            is not None
        )
    ...
    def test_get_core_site_packages_regex_invalid_paths(self):
        ...
        assert (
            regex.search(
                "/opt/splunk/lib/python3.9/site-packages/pypng-0.0.20-py3.9.egg"
            )
            is None
        )
        assert (
            regex.search("/opt/splunk/lib/python3.9/site-packages/IPy-1.0-py3.9.egg")
            is None
        )
        assert (
            regex.search(
                "/opt/splunk/lib/python3.9/site-packages/bottle-0.12.25-py3.9.egg"
            )
            is None
        )
...
class TestGetAppPath:
    ...
    def test_get_app_path_success_cases(
    ...
    @pytest.mark.parametrize(
        ...
        [
            # No etc/apps in path
            ("/home/user/myproject/lib/mymodule.py", "lib"),
            # etc/apps exists but script folder doesn't exist after it
            ("/opt/splunk/etc/apps/my_app/default/app.conf", "lib"),
            # Script folder exists but not after etc/apps
            ("/home/user/lib/myproject/etc/apps/config", "lib"),
            # Empty path
            ("", "lib"),
        ],
    )
    ...
    def test_get_app_path_default_current_script_folder(self):
        """Test that _get_app_path uses 'lib' as default
    ...
    def test_get_app_path_windows_success_cases(
        self, absolute_path, current_script_folder, expected_result
    ):
        """Test successful cases of _get_app_path function with Windows
        paths."""
        with patch("os.path.join") as mock_join:
            # Mock os.path.join to return Windows-style paths
            mock_join.return_value = "\\etc\\apps"

            result = core_lib_handler._get_app_path(
                absolute_path, current_script_folder
            )
            assert result == expected_result
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("C:\\Users\\user\\myproject\\lib\\mymodule.py", "lib"),
            # etc\\apps exists but script folder doesn't exist after it - Windows
            ("C:\\Program Files\\Splunk\\etc\\apps\\my_app\\default\\app.conf", "lib"),
            # Script folder exists but not after etc\\apps - Windows
            ("C:\\Users\\user\\lib\\myproject\\etc\\apps\\config", "lib"),
            # Empty path - Windows
            ("", "lib"),
        ],
    )
    def test_get_app_path_windows_returns_none(
        self, absolute_path, current_script_folder
    ):
        """Test cases where _get_app_path should return None with Windows
        paths."""
        with patch("os.path.join") as mock_join:
            # Mock os.path.join to return Windows-style paths
            mock_join.return_value = "\\etc\\apps"
    ...
    def test_get_app_path_windows_multiple_script_folders(self):
        """Test _get_app_path when script folder appears multiple times on
        Windows."""
        with patch("os.path.join") as mock_join:
            # Mock os.path.join to return Windows-style paths
            mock_join.return_value = "\\etc\\apps"
